...
offer_form.onsubmit = async (e) => {
  ...
  if (offer_form.checkValidity() === true) {
    ...
    console.log(str2);
    let organisation = document.getElementById('organisation_name').value;
    let profile = document.getElementById('profile').value;
    let description = document.getElementById('description').value
    let intake = document.getElementById('intake').value
    let grades = document.getElementById('grades').value
    let path = 'api/placement?organisation='+organisation+'&profile='+profile+
        '&description='+description+'&intake='+intake+'&minimum_grade='+grades+
        '&specialisation='+str1+'&domain='+str2;
    console.log(path)
